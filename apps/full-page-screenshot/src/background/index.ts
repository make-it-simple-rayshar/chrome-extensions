...
async function injectContentScript(tabId: number): Promise<void> {
  try {
    ...
    console.log(`[FPS] Content script injected into ${results?.length ?? 0} frames`);
  } catch {
...
async function handleIframeCapture(
  ...
  if (!iframeFrame) {
    ...
    console.log('[FPS] Iframe frame not found, falling back to viewport crop');
    await captureElement(targetRect);
    return;
  }

  const iframeFrameId = iframeFrame.frameId;
  console.log(`[FPS] Resolved iframe frameId=${iframeFrameId} for ${iframeSrc}`);
